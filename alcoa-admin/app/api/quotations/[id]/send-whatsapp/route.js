...
/** International digits only (wa.me expects no + or spaces). */
...
export const POST = withErrorHandler(async (request, context) => {
  ...
  if (!isWhatsAppFeatureAvailable() && !(useTwilio && localTextOnly && twilioReady)) {
    return apiError(
      "WhatsApp is not available. Add FEATURES=whatsapp (or SERVER_FEATURES), set BLOB_READ_WRITE_TOKEN for PDF hosting, and for Twilio set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN. Set WHATSAPP_USE_TWILIO=false to use Blob + wa.me only (no Twilio).",
      503
    );
  }
  ...
  if (useTwilio) {
    if (!twilioReady) {
      throw new AppError(
        "WhatsApp Twilio mode is on but Twilio is not configured. Set TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN, or set WHATSAPP_USE_TWILIO=false to use Blob + wa.me instead.",
        503
      );
    }
  ...
  // ─── Blob URL + wa.me (no Twilio) ──────────────────────────────────────
  if (!isBlobReadWriteTokenConfigured()) {
    throw new AppError(
      "wa.me mode requires Vercel Blob. Set BLOB_READ_WRITE_TOKEN (Vercel → Storage → Blob → .env.local tab).",
      503
    );
  }
