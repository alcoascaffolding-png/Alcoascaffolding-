...
function ReceiptFormFields({ control }) {
  ...
  const openInvoices = (invoicesData?.items || []).filter((inv) => {
    if (inv.paymentStatus === "cancelled" || inv.paymentStatus === "paid") return false;
    return openInvoiceBalance(inv) > 0.01;
  });

  const invoiceOptions = openInvoices.map((inv) => ({
    value: inv._id,
    label: `${inv.invoiceNumber} — ${inv.customerName} (due ${formatCurrency(openInvoiceBalance(inv))})`,
  }));
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      ...
      <FormTextField control={control} name="receiptDate" label="Receipt date" type="date" />
