...
export default function salesOrderEmailTemplate(order, options = {}) {
  ...
  const itemRows = (order.items || [])
    ...
    <tr style="background:${i % 2 === 0 ? "#ffffff" : "#f8fafc"};">
      ...
      <td style="padding:10px 12px;border-bottom:1px solid #e2e8f0;font-size:13px;text-align:right;">${formatCurrency(item.unitPrice, order.currency)}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #e2e8f0;font-size:13px;text-align:right;font-weight:600;">${formatCurrency(item.total, order.currency)}</td>
    </tr>`
    )
    .join("");
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Sales Order ${order.orderNumber}</title></head>
<body style="font-family:Inter,ui-sans-serif,system-ui,-apple-system,'Segoe UI',Roboto,Arial,sans-serif;line-height:1.55;color:${b.text};margin:0;padding:24px 12px;background:#e8eef5;">
  <div style="max-width:700px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(29,58,108,0.12);border:1px solid ${b.accentBorder};">
    ...
    <div style="padding:24px;">
      <p style="margin:0 0 8px;font-size:15px;">Dear ${order.customerName},</p>
      <p style="margin:0 0 18px;color:${b.muted};">Please find your <strong>sales order</strong> below. A PDF copy is attached.</p>
      <div style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:18px;font-size:13px;color:${b.muted};">
        <span><strong style="color:${b.text};">Order</strong> ${order.orderNumber}</span>
        <span>·</span>
        <span><strong style="color:${b.text};">Date</strong> ${formatDate(order.orderDate)}</span>
      </div>
      ...
      <div style="margin-top:16px;text-align:right;font-size:14px;">
        <div><span style="color:${b.muted};">Subtotal</span> <strong>${formatCurrency(order.subtotal, order.currency)}</strong></div>
        <div><span style="color:${b.muted};">VAT</span> <strong>${formatCurrency(order.vatAmount, order.currency)}</strong></div>
        <div style="margin-top:8px;font-size:16px;"><span style="color:${b.muted};">Total</span> <strong style="color:${b.primary};">${formatCurrency(order.total, order.currency)}</strong></div>
      </div>
      ${order.notes ? `<p style="margin-top:20px;font-size:13px;color:${b.muted};white-space:pre-wrap;"><strong>Notes</strong><br/>${order.notes}</p>` : ""}
